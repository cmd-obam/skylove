...
Deno.serve(
  withCors(async (req) => {
    ...
    const { data: verified, error: rpcError } = await adminClient.rpc('verify_password_recovery_answer', {
      p_name: name,
      p_email: email,
      p_answer: answer,
    })
    ...
    if (!rpcError) {
      return jsonResponse({ success: Boolean(verified) })
    }
    ...
    const matched = await compare(answer, profile.security_answer_hash)

    return jsonResponse({ success: matched })
  }),
)
