...
$srcPath = Join-Path $PSScriptRoot '..\src\assets\images\about\pastor-oval-signature-source.png'
$outPath = Join-Path $PSScriptRoot '..\src\assets\images\about\pastor-signature-text.png'
...
for ($x = 0; $x -lt $src.Width; $x++) {
  for ($y = 0; $y -lt $src.Height; $y++) {
    if (-not (Test-TransparentPixel $src.GetPixel($x, $y))) {
      if ($x -lt $minX) { $minX = $x }
      if ($y -lt $minY) { $minY = $y }
      if ($x -gt $maxX) { $maxX = $x }
      if ($y -gt $maxY) { $maxY = $y }
...
Write-Host "Saved trimmed signature ${cropWidth}x${cropHeight} to $outPath"
