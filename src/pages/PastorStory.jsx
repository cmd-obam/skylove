...
function PastorStory() {
  const [searchKeyword, setSearchKeyword] = useState('')
  const [searchQuery, setSearchQuery] = useState('')
  const { posts, loading } = useBoardPostList(POST_TYPE)
    ...
    <div className="church-news-page">
      <header className="sub-layout__header">
        <div className="sub-layout__heading">
          <h1 className="sub-layout__title">담임목사 이야기</h1>
          <p className="sub-layout__subtitle">담임목사의 이야기를 전합니다.</p>
        </div>
        <Breadcrumb />
      </header>
      ...
      <div className="church-news-board__toolbar">
        <form className="church-news-board__search" onSubmit={handleSearchSubmit} autoComplete="off">
          <label htmlFor="pastor-story-search" className="visually-hidden">
            제목 검색
          </label>
          <input
            id="pastor-story-search"
            type="search"
        ...
        <BoardWriteButton to="/pastor-story/write" postType={POST_TYPE} />
      </div>
