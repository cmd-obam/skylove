...
const LIST_PATH = '/church-news/event-photos'
...
function EventPhotos() {
  ...
  const filteredPosts = EVENT_PHOTO_POSTS.filter((post) => {
    ...
    const matchesSearch = matchesEventPhotoSearch(post, searchQuery, activeSearchType)
    ...
    <div className="church-news-page">
      <header className="church-news-page__header">
        <div className="church-news-page__heading">
          <h1 className="church-news-page__title">행사사진</h1>
      ...
      <section className="church-news-board" aria-label="행사사진 게시판">
        <div className="event-photos-toolbar">
          <div className="event-photos-filter" role="group" aria-label="게시글 분류">
            {EVENT_PHOTO_CATEGORIES.map((category) => (
              <button
                key={category}
                type="button"
                className={`event-photos-filter__button${
                  activeCategory === category ? ' event-photos-filter__button--active' : ''
                }`}
