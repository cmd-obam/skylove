 ...
 * Reuses the signup leave-guard pattern (click capture + popstate + beforeunload).
 */
export function useUnsavedLeaveGuard({ isDirty, onConfirmLeave }) {
  ...
  useEffect(() => {
    ...
    window.history.pushState({ unsavedLeaveGuard: true }, '')
